...
void GPIO_Enable(Pin_t pin); //Enables the system clock to a GPIO peripheral via RCC_AHB2ENR

void GPIO_PortEnable(GPIO_TypeDef *port);

void GPIO_Mode(Pin_t pin, PinMode_t mode); //Configures the mode of a GPIO pin via GPIOx_MODER

void GPIO_Config(Pin_t pin, PinType_t ot, PinSpeed_t osp, PinPUPD_t pupd);

void GPIO_AltFunc(Pin_t pin, int af);

PinState_t GPIO_Input(Pin_t pin); //Returns the state of a GPIO input pin by reading GPIOx_IDR

uint16_t GPIO_PortInput(GPIO_TypeDef *port);

void GPIO_Output(Pin_t pin, PinState_t state); //Drives a GPIO output pin to the specified state using GPIOx_BSRR

void GPIO_PortOutput(GPIO_TypeDef *port, uint16_t states);

void GPIO_Toggle(Pin_t pin); //Toggles the state of a GPIO output pin by manipulating GPIOx_ODR
