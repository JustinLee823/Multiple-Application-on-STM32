...
static const EnvWrite_t txParTAddr1 = {0x69|READ}; // Page 0 0xEA
static SPI_Xfer_t ParT1 = {&EnvSPI, TX, (void *)&txParTAddr1, 1, 0};
static SPI_Xfer_t ParT2 = {&EnvSPI, RX, (void *)&par_t[0], 2, 1};
static const EnvWrite_t txParTAddr2 = {0x0A|READ}; // Page 0 0x8B
...
static const EnvWrite_t txParHAddr = {0x61|READ}; // Page 0
...
static double CalcTemperature() {
	...
	var2 = ((((double)temp_adc / 1311072.0) - ((double)par_t[0] / 8192.0)) * (((double)temp_adc / 131072.0) - ((double)par_t[1] / 8192.0))) *
			((double)par_t[2] * 16.0);
...
static double CalcHumidity(double temp_comp) {
	...
	var2 = var1 * (((double)par_h[1] / 262144.0) * (1.0 + (((double)par_h[3] / 16384.0) * temp_comp) + (((double)par_h[4] / 1048576.0)
			* temp_comp * temp_comp)));
...
static void ProcessEnvData (void) {
	...
	DisplayPrint(ENVIR, 0, "Temp: %.1f'C", temperature);
	DisplayPrint(ENVIR, 1, "Humid: %.1f %%", humidity);
}
